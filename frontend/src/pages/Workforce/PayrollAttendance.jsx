...
export default function StaffHub() {
    ...
    const handleDeleteStaff = async (id, name) => {
        if (!window.confirm(`Are you sure you want to remove ${name} from your staff? This action cannot be undone.`)) return;
    ...
    return (
        <div className="space-y-8 animate-fade-in-up pb-10 max-w-[1400px] mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="p-8 bg-white shadow-xl rounded-[32px] relative overflow-hidden group">
                ...
                <div className="p-8 bg-emerald-500 shadow-xl shadow-emerald-200 rounded-[32px] text-white">
            ...
            <div className="flex bg-slate-100 p-2 rounded-[24px] w-fit shadow-inner mx-auto mb-10 border border-slate-200">
                {['list', 'salary-history', 'attendance'].map(tab => (
                    <button
                        ...
                        className={`px-10 py-4 rounded-[18px] text-[15px] font-black transition-all ${activeTab === tab ? 'bg-white text-brand-navy shadow-xl' : 'text-slate-400 hover:text-brand-navy'
                            }`}
                    >
                                                ...
                                                <Td className="text-right px-10">
                                                    <div className="flex items-center justify-end gap-4">
                                                        {payroll.some(p => p.staff_id === s.id && p.payment_type !== 'advance' && new Date(p.payment_date).toDateString() === new Date().toDateString()) ? (
                                                            <div className="flex items-center gap-2 text-emerald-600 font-black text-[13px] bg-emerald-50 px-4 py-2 rounded-xl">
                                                                <CheckCircle size={16} /> Paid
                                                            </div>
                                                        ) : (
                                                            <button onClick={() => { setSelectedStaff(s); setShowPayModal(true); }} className="text-brand-blue font-black text-[14px] hover:underline flex items-center gap-2">Pay Salary <ArrowRight size={14} /></button>
                                                        )}
                                                        <button
                                ...
                                <div className="space-y-1 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar -mx-2">
                                    {staff.filter(s => s.name && s.name.toLowerCase().includes(calSearch.toLowerCase())).map(s => (
                                        <button
                                            ...
                                            className={`w-full text-left p-4 rounded-xl font-black text-[14px] transition-all flex items-center justify-between group relative overflow-hidden ${calStaffId === s.id ? 'bg-brand-blue/5 text-brand-blue' : 'text-slate-500 hover:bg-slate-50'
                                                }`}
                                        >
                            ...
                            <Card className="xl:col-span-3 p-10 border-none bg-white shadow-2xl rounded-[48px] relative overflow-hidden">
                                ...
                                <div className="grid grid-cols-7 gap-4 mb-12">
                                    ...
                                    {getDaysInMonth(calMonth, calYear).map(day => {
                                        ...
                                        return (
                                            <div key={dateStr} className={`h-[100px] border-2 rounded-[32px] p-5 transition-all relative group flex flex-col justify-between ${log ? 'bg-white border-brand-blue/20 shadow-xl' :
                                                ...
                                                {log ? (
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-3.5 h-3.5 rounded-full shadow-lg ${log.status === 'present' ? 'bg-emerald-500 shadow-emerald-200' :
